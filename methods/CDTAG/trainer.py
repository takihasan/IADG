...
from losses import MaxEntropyLoss, OrthogonalityLoss, ContrastiveLoss
...
class Trainer:
    def __init__(self, model, loaders, config, domain):
        ...
        self.loss_fn_adv = MaxEntropyLoss()
        self.loss_fn_orth = OrthogonalityLoss()
        self.loss_fn_contr = ContrastiveLoss()
        ...
        # training progress
        self.accuracy_dev_best = None
        self.loss_train_best = None
        self.loss_dev_best = None
    ...
    def _training_status(self):
        message = {}
        message["loss_dev_best"] = f"{self.loss_dev_best: .2f}"
        message["acc_dev_best"] = f"{self.accuracy_dev_best*100 :.1f}"
        return message
    ...
    def train_iter(self):
        ...
        # featurize
        emb_f = self.model.featurizer(all_inputs)
        # encode
        emb_label = self.model.encoder_label(emb_f)
        emb_domain = self.model.encoder_domain(emb_f)

        # project
        proj_label = self.model.projector_label(emb_label)
        proj_domain = self.model.projector_domain(emb_domain)

        # classify
        logit_label = self.model.classifier_label(emb_label)
        logit_domain = self.model.classifier_domain(emb_domain)
        # adversarial classify
        logit_domain_on_label = self.model.classifier_label(emb_domain)

        # loss label classification
        loss_cls_label = self.loss_fn_cls(logit_label, all_labels)
        # loss domain classification
        loss_cls_domain = self.loss_fn_cls(logit_domain, all_domains)
        # orthogonality
        loss_orth = self.loss_fn_orth(emb_label, emb_domain)
        # loss max entroppy
        loss_adv = self.loss_fn_adv(logit_domain_on_label, all_labels)
        # loss contrastive
        ...
        loss = 0.0
        loss = loss + loss_cls_label * 1.0
        loss = loss + loss_cls_domain * 1.0
        loss = loss + loss_orth * 1.0
        loss = loss + loss_adv * 1.0
        ...
        tracking = {
            ...
            "loss_cls_label": loss_cls_label.item(),
            "loss_cls_domain": loss_cls_domain.item(),
            "loss_cls_orth": loss_orth.item(),
            "loss_cls_adv": loss_adv.item(),
        ...
        # update best model
        if self.loss_train_best is None or self.loss_train_best > loss.item():
            self.loss_train_best = loss.item()
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            for loader in self.loader_dev.loaders:
                ...
                for inputs, labels in loader:
                    ...
                    # classify
                    logit_label = self.model(inputs)
                    # loss
                    loss += self.loss_fn_cls(logit_label, labels) * len(inputs)
                    # predictions
        ...
        # update best loss
        if self.loss_dev_best is None or loss < self.loss_dev_best:
            self.loss_dev_best = loss
            name = f"weights_best_loss_{self.iter_count:05d}"
            self.model_best_loss_name = name
            self.save(name, save_optimizer=False)

        # update best model
        if self.accuracy_dev_best is None or accuracy > self.accuracy_dev_best:
            self.accuracy_dev_best = accuracy
            name = f"weights_best_acc_{self.iter_count:05d}"
            self.model_best_acc_name = name
            self.save(name, save_optimizer=False)

    def test(self, loader=None, load_best=True):
        """
        Run testing.
        """
        if loader is None:
    ...
    def load_best(self, load_device=None):
        ...
        Load best model.
        """
        model_best_name = self.model_best_loss_name
        savepath = os.path.join(self.dir_log, model_best_name + ".pth")
        if load_device is None:
            load_device = self.device
